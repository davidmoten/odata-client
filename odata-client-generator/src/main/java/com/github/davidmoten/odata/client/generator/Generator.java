...
public final class Generator {
    ...
    private void writeCollectionRequest(Schema schema, TEntityType entityType) {
        Structure<TEntityType> t = new EntityType(entityType, names);
        t.getDirectoryCollectionRequest().mkdirs();
        String simpleClassName = t.getSimpleClassNameCollectionRequest();
        Imports imports = new Imports(simpleClassName);
        Indent indent = new Indent();

        StringWriter w = new StringWriter();
        try (PrintWriter p = new PrintWriter(w)) {
            p.format("package %s;\n\n", t.getPackageCollectionRequest());
            p.format("IMPORTSHERE");
                    ...
                    .forEach(x -> {
                        ...
                        if (x.getType().get(0).startsWith(COLLECTION_PREFIX)) {
                            ...
                            p.format("%spublic %s %s() {\n", //
                                    indent, //
                                    imports.add(names
                                            .getFullClassNameEntityCollectionRequestFromTypeWithNamespace(
                                                    sch, y)), //
                                    Names.getIdentifier(x.getName()));
                                        ...
                                        if (names.isCollection(returnParameter.get())) {
                                            p.format("\n%s%s %s(%s) {\n", //
                                                    ...
                                                    names.getFullClassNameEntityCollectionRequestFromTypeWithNamespace(
                                                            names.getSchema(returnInnerType),
                                                            returnInnerType),
                            ...
                            if (SET_VIA_CONSTRUCTOR) {
                                ...
                                if (ofEntity) {
                                    params = "contextPath, changedFields.add(\"" + x.getName()
                                            + "\")" + ", unmappedFields, "
                                            + imports.add(
                                                    com.github.davidmoten.odata.client.Util.class)
    ...
    private void printPropertyFields(Imports imports, Indent indent, PrintWriter p,
        ...
        if (!hasBaseType) {
            ...
            p.format("%sprotected %s%s %s;\n", indent, finalOrBlank, imports.add(String.class),
                    "odataType");
        }
        properties.stream().forEach(x -> {
            p.format("\n%s@%s(\"%s\")\n", indent, imports.add(JsonProperty.class), x.getName());
            p.format("%sprotected %s%s %s;\n", indent, finalOrBlank,
                    names.toImportedType(x, imports), Names.getIdentifier(x.getName()));
